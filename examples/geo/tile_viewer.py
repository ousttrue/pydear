...
class XYZTile(Item):
    def __init__(self, loop: asyncio.AbstractEventLoop, base_url) -> None:
        ...
        self.texture_manager = TileTextureManager(
            loop, base_url, HERE.parent.parent / "tile_cache"
        )
    ...
    def mouse_drag(
        self, x: int, y: int, dx: int, dy: int, left: bool, right: bool, middle: bool
    ):
        if middle:
...
def main():
    ...
    log_handler.setFormatter(
        logging.Formatter("%(name)s:%(lineno)s[%(levelname)s]%(message)s")
    )
    ...
    def show_view(p_open):
        ...
        if ImGui.Begin(
            ...
            texture = fbo_manager.clear(int(w), int(h), clear_color)
            if texture:

                ImGui.ImageButton(texture, (w, h), (0, 1), (1, 0), 0, bg, tint)
                ImGuiInternal.ButtonBehavior(
    ...
    views = [
        dockspace.Dock("demo", ImGui.ShowDemoWindow, (ctypes.c_bool * 1)(True)),
        dockspace.Dock("metrics", ImGui.ShowMetricsWindow, (ctypes.c_bool * 1)(True)),
        dockspace.Dock("hello", show_hello, (ctypes.c_bool * 1)(True)),
        dockspace.Dock("view", show_view, (ctypes.c_bool * 1)(True)),
        dockspace.Dock("log", log_handler.show, (ctypes.c_bool * 1)(True)),
    ]
